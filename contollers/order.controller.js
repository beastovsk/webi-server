...
const orderController = {
	createOrder: async (req, res) => {
		try {
			...
			await db.query(
				`INSERT INTO "order" (shop_id, phone, name, email, address, products) VALUES ($1, $2, $3, $4, $5, $6)`,
	...
	getOrders: async (req, res) => {
		try {
			...
			const currentOrder = await db.query(
				`SELECT * FROM "order" WHERE shop_id = $1`,
				[shopId]
			);
	...
	getOrderById: async (req, res) => {
		try {
			...
			const currentOrder = await db.query(
				`SELECT * FROM "order" WHERE shop_id = $1 AND id = $2`,
				[shopId, orderId]
			);
